...
export class AssetsService {
  ...
  constructor() {
  this.assets.forEach((o,i)=>o['id']=i);
  }

  getAssets(filters?: any) {
    if(filters) {
      let filteredAsset = [...this.assets];
      if(filters.RAM.length) {
        filteredAsset = filteredAsset.filter(e => {
          for(let fram of filters.RAM) {
            if(e.RAM.includes(fram)) {
              return true;
            } else {
              return false
            }
      ...
      if(filters.Location) {
        filteredAsset = filteredAsset.filter(e => filters.Location === e.Location);
      }
      if(filters.HDD) {
        filteredAsset = filteredAsset.filter(e => e.HDD.includes(filters.HDD));
      }
  ...
  addAsset(asset: Asset) {
    asset['id'] = this.assets.length;
    this.assets.push(asset);
    this.assetsChanged.next(this.assets.slice());
  }

  updateAsset(id: number, newAsset: Asset) {
    let index = this.assets.findIndex(x => x['id'] == id);
    newAsset['id'] = id;
    this.assets[index] = newAsset;
    this.assetsChanged.next(this.assets.slice());
  }
  ...
  getLocations() {
    // find and return uniq locations
     return [...new Set(this.assets.map(item => item.Location))];
  }
}
